...
merge 模式不是信仰,是 2026-09 决策时的数据最优解:近 6 个月上游 171 个提交,
原样消费 85 个(≈50%,含 h264bitstream SPS 修复、common-c 丢帧修复等自研难
覆盖的共享路径维护),自己重实现或无关 52 个。出现以下任一信号,重启脱离
评估,并用当期数据与基线对比:

1. **成本触发**:单次 merge 的冲突处理超过半天,或 session.cpp 冲突面显著
   膨胀(基线:2026-09 净 +3510 行,单次 merge 半天内可完成)。
2. **价值触发**:连续两个季度「自己重实现 + 无关」占比超过 60%
   (基线:2026-09 为 ≈50%,统计口径见当期决策记录)。
3. **方向触发**:上游做出与 V+ 目标不兼容的迁移(如 Qt 7、构建系统更换),
   跟随成本高于脱离成本。

届时脱离的默认形式是「断 merge、断 Git 关系、留四根脐带」:安全瞭望 SLA 化、
继续消费上游 deps 产物、协议库周期 diff、cherry-pick 门常开——不是推倒重写。
判定数据每月可由 upstream-status workflow 的存档输出复盘。
